...
function StarReview({ auth }) {
  ...
  const [hasCompletedFeedbacks, setCompletedFeedbacks] = useState(false);
  ...
  useEffect(() => {
    ...
    if (auth) {
      ...
      if (!auth.hasReviews && hasCompletedFeedbacks) {
        setVisibility('visible')
        if (toggleReviewDiv) {
          toggleReviewDiv.style.visibility = 'hidden';
        }
      } else if (hasCompletedFeedbacks){
        if (toggleReviewDiv) {
  ...
  }, [auth, hasCompletedFeedbacks]);

  const fetchItems = async () => {
    const userData = await fetch(`/api/Feedbacks/completed`);
    const items = await userData.json();
    const hasCompletedFeedbacks = items && items.length > 0 ? true : false
    setCompletedFeedbacks(hasCompletedFeedbacks);
...
function mapStateToProps({ auth, feedbacks }) {

  return { auth, feedbacks };
}
